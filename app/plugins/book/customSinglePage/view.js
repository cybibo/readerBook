define(function (require, exports, module) {
  ...
  var pageTpl = require("tpl!./flatpage");
  /**
   ...
   * @type {Function}
   */
  var BookFlip = require("./bookslide");
  ...
  var SinglePageView = PluginBase.View.extend({
    ...
    endInit: function (ebook) {
      ...
      this.BookFlip = new BookFlip({
        onstart: this.prepareToTurnPage.bind(this),
        onend: this.preloadPages.bind(this),
        item: ".page",
      ...
      this.addPage(config.get('start'), function () {
        this.preloadPages(config.get('start')-1, config.get('start'));
    ...
    toPage: function (page) {
      ...
      if (!isNaN(page) && this.$pageList.eq(page - 1)[0].children.length === 0) {
        this.addPage(page, this.animateToPage.bind(this));
      } else {
        this.animateToPage(page);
      }

    },
    animateToPage: function(page) {
     ...
     * @param newPage
     */
    preloadPages: function (oldPage, newPage) {
    ...
    setPageCache: function (p, pageContent) {
      var $pageContent = this.$pageList.eq(p - 1);
      var $content = $("<div></div>").append(pageContent);
      console.log("---> page " + p + " loaded.");
      this.emit("unit:add", p, new Unit($content, p));
    ...
    addBlankPages: function () {
      ...
      var html      = "";
      var templateData, page, pageId;

      for (var i = 1; i <= pageCount; i++) {
        templateData = {};

        templateData.isLastPage = 0;
        if (i === pageCount ) {
          templateData.isLastPage = 1;
        }
        
        templateData.pageNumber = i;
        pageId = orders[i - 1];
        if (pageId) {
          page = points[pageId];
          templateData.module = page.module;
          templateData.unit = page.unit;
          templateData.id = page.id;
          templateData.pages = modules[page.module][page.unit].pages;
        }

        html += pageTpl(templateData);
     ...
     */
    prepareToTurnPage: function (oldPage, newPage) {
